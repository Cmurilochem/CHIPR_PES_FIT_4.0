...
C      WRITE(*,50)
C      WRITE(*,70)
      WRITE(123,1000) N, MAX, T, RT, EPS, NS, NT, NEPS, MAXEVL, IPRINT,
     1              ISEED1, ISEED2
      ...
      WRITE(123,'(/,''  ****   END OF DRIVER ROUTINE OUTPUT   ****''
     1          /,''  ****   BEFORE CALL TO SA.             ****'')') 
     ...
     2        FSTAR,XP,NACP)
      ...
      WRITE(123,'(/,''  ****   RESULTS AFTER SA   ****   '')') 
      CALL PRTVEC(XOPT,N,'SOLUTION')
      CALL PRTVEC(VM,N,'FINAL STEP LENGTH')
      WRITE(123,1001) FOPT, NFCNEV, NACC, NOBDS, T, IER

C  50  FORMAT ("****************************************************")
C  70  FORMAT(/,'4). STARTING SIMULATED ANNEALING',/)
1000  FORMAT(/,"***** SIMULATED ANNEALING RUN *****",/,
...
C  return to the calling routine. 
      IF (T .LE. 0.0) THEN
         WRITE(123,'(/,''  THE INITIAL TEMPERATURE IS NOT POSITIVE. ''
...
C                                                      0 <= KL <= 30081
      real U(97), C, CD, CM
      integer I97, J97, IJ, KL, I, J, K, L, II
      integer M, JJ
      real T,S,UNI
...
C  it minimizes by maximizing the negative a function.
      ...
      WRITE(123,'(/,''  THE STARTING VALUE (X) IS OUTSIDE THE BOUNDS ''
     ...
     3          /,''  LB(I) .LT. X(I) .LT. UB(I), I = 1, N. ''/)')
      ...
      IF (MAX) THEN
         WRITE(123,'(''  CURRENT F: '',G25.18)') F
      ELSE
         WRITE(123,'(''  CURRENT F: '',G25.18)') -F
      END IF
      CALL PRTVEC(XP,N,'TRIAL X')
      WRITE(123,'(''  POINT REJECTED SINCE OUT OF BOUNDS'')')
      ...
      IF (MAX) THEN
         WRITE(123,'(''  CURRENT F: '',G25.18)') F
         CALL PRTVEC(XP,N,'TRIAL X')
         WRITE(123,'(''  RESULTING F: '',G25.18)') FP
      ELSE
         WRITE(123,'(''  CURRENT F: '',G25.18)') -F
         CALL PRTVEC(XP,N,'TRIAL X')
         WRITE(123,'(''  RESULTING F: '',G25.18)') -FP
      ...
      WRITE(123,'(/,''  TOO MANY FUNCTION EVALUATIONS; CONSIDER ''
     ...
     3          /,''  POOR.'',/)')
      ...
      IF (MAX) THEN
         WRITE(123,'(''  THOUGH LOWER, POINT ACCEPTED'')')
      ELSE
         WRITE(123,'(''  THOUGH HIGHER, POINT ACCEPTED'')')
      END IF
      ...
      IF (MAX) THEN
         WRITE(123,'(''  LOWER POINT REJECTED'')')
      ELSE
         WRITE(123,'(''  HIGHER POINT REJECTED'')')
      END IF
     ...
     1  '' INTERMEDIATE RESULTS BEFORE NEXT TEMPERATURE REDUCTION'',/)')
      WRITE(123,'(''  CURRENT TEMPERATURE:            '',G12.5)') T
      IF (MAX) THEN
         WRITE(123,'(''  MAX FUNCTION VALUE SO FAR:  '',G25.18)') FOPT
         WRITE(123,'(''  TOTAL MOVES:                '',I8)') TOTMOV
         WRITE(123,'(''     UPHILL:                  '',I8)') NUP
         WRITE(123,'(''     ACCEPTED DOWNHILL:       '',I8)') NDOWN
         WRITE(123,'(''     REJECTED DOWNHILL:       '',I8)') NREJ
         WRITE(123,'(''  OUT OF BOUNDS TRIALS:       '',I8)') LNOBDS
         WRITE(123,'(''  NEW MAXIMA THIS TEMPERATURE:'',I8)') NNEW
      ELSE
         WRITE(123,'(''  MIN FUNCTION VALUE SO FAR:  '',G25.18)') -FOPT
         WRITE(123,'(''  TOTAL MOVES:                '',I8)') TOTMOV
         WRITE(123,'(''     DOWNHILL:                '',I8)')  NUP
         WRITE(123,'(''     ACCEPTED UPHILL:         '',I8)')  NDOWN
         WRITE(123,'(''     REJECTED UPHILL:         '',I8)')  NREJ
         WRITE(123,'(''  TRIALS OUT OF BOUNDS:       '',I8)')  LNOBDS
         WRITE(123,'(''  NEW MINIMA THIS TEMPERATURE:'',I8)')  NNEW
      END IF
      CALL PRTVEC(XOPT,N,'CURRENT OPTIMAL X')
      ...
      WRITE(123,'(/,''SA ACHIEVED TERMINATION CRITERIA. IER = 0.'',/)')
...
C  elements in VECTOR, 10 elements will be printed on each line.
      ...
      IF (NCOLS .GT. 10) THEN
         ...
         DO 100, I = 1, LINES
            LL = 10*(I - 1)
            WRITE(123,1000) (VECTOR(J),J = 1+LL, 10+LL)
  100    CONTINUE

         WRITE(123,1000) (VECTOR(J),J = 11+LL, NCOLS)
      ELSE
         WRITE(123,1000) (VECTOR(J),J = 1, NCOLS)
      END IF
