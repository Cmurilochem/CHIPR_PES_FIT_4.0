C############################################################################### MINPACK RELATED
     ...
     *                 wa1(n),wa2(n),wa3(n),wa4(m),dfvec(m,n)
      external fcn
c     **********
c
c     subroutine lmdif
...
c
      iflag = 1
      call fcn(id,m,n,x,fvec,dfvec,iflag)
      nfev = 1
...
c
         iflag = 2
c
c        IN THIS CASE, CALCULATES IT SEMI-NUMERICALLY
c         
         call SEMI_NUM_fdjac2(fcn,m,n,x,fvec,
     *                        fjac,ldfjac,iflag,epsfcn,wa4)
c         call fdjac2(fcn,m,n,x,fvec,
c     *                        fjac,ldfjac,iflag,epsfcn,wa4)     
         nfev = nfev + n         
         if (iflag .lt. 0) go to 300
c
...
c
         ...
         iflag = 0 
         if (mod(iter-1,nprint).eq.0)call fcn(id,m,n,x,fvec,dfvec,iflag)
         if (iflag .lt. 0) go to 300
   40    continue
c
...
c
            iflag = 1
            call fcn(id,m,n,wa2,wa4,dfvec,iflag)
            nfev = nfev + 1
            if (iflag .lt. 0) go to 300
            fnorm1 = enorm(m,wa4)
c
...
c
      ...
      iflag = 0
      if (nprint .gt. 0) call fcn(id,m,n,x,fvec,dfvec,iflag)
      return
c
...
c
      ...
      integer m,n,ldfjac,iflag,id
      double precision epsfcn
      double precision x(n),fvec(m),fjac(ldfjac,n),wa(m)
      double precision dfvec(m,n)      
c     **********
c
c     subroutine fdjac2
c
...
c
      ...
      do 20 j = 1, n
         ...
         if (h .eq. zero) h = eps
         x(j) = temp + h
         call fcn(id,m,n,x,wa,dfvec,iflag)
   ...
   20    continue    
   30 continue
c          print*, fjac(1,1),fjac(1,2)
c          print*, fjac(1,12),fjac(1,13)
      return
...
C###############################################################################
      ...
      subroutine SEMI_NUM_fdjac2(fcn,m,n,x,fvec,
     *                           fjac,ldfjac,iflag,epsfcn,wa)
      USE COMMON_VAR, ONLY : OPTTYP,JOBTYP,MOLTYP
      USE COMMON_VAR, ONLY : NCPOL,NX
      integer m,n,ldfjac,iflag,id
      double precision epsfcn
      double precision x(n),fvec(m),fjac(ldfjac,n),wa(m),waa(m)
      double precision dfvec(m,n),dfvec1(m,n),dfvec2(m,n)
c     **********
c
c     subroutine fdjac2
c
...
c
c     CALCULATING THE JACOBIAN MATRIX ANALYTICALLY
c
          call fcn(id,m,n,x,wa,dfvec,iflag)
          if (iflag .lt. 0) go to 30
   ...
   30     continue
        ...
        ELSE 
c
c     CALCULATING THE JACOBIAN MATRIX SEMI-NUMERIC
c     ANALITICALLY FOR LARGE Cs
c     NUMERIC FOR BASIS SET CONTRACTION COEFFICIENTS
c        
          call fcn(id,m,n,x,waa,dfvec1,iflag)
          if (iflag .lt. 0) go to 33
          
          do j = 1, NCPOL   
             do i = 1, m
               fjac(i,j) = dfvec1(i,j)/x(j)
             end do
          end do   
          ...
          do j = NCPOL+1, n
             ...
             if (h .eq. zero) h = eps
             x(j) = temp + h
             call fcn(id,m,n,x,wa,dfvec2,iflag)
             if (iflag .lt. 0) go to 33
             x(j) = temp        
             do i = 1, m
               fjac(i,j) = (wa(i) - fvec(i))/h
   ...
   33     continue  
        ...
        END IF   
c
c     FOR BASIS SET CALIBRATION AND DIRECT FITS CALCULE THE JACOBIAN MATRIX NUMERICALLY
c      
      ELSE 
